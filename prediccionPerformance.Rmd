...
```{r exploración}
datos <- mutate( datos, frecuencia = 10^5 * 1/ciclo )
...
varianzaObjetivo <- function(lambda) {
  ...
  ajustados <- ( modeloLineal$fitted.values )^(1/lambda)
  varianza <- ( var(ajustados) )
  return(varianza)
}
...
```{r ¿qué estoy midiendo?}
modeloPrueba <- lm( perfoRelativa^2 ~ minMem + frecuencia, datos )
datos$ajustadosP <- modeloPrueba$fitted.values 
datos$residuosP <- modeloPrueba$residuals
datos$perfoCuadrado <- ( modeloPrueba$perfoRelativa )^2
```

# PENDIENTE:

Mirar qué pasa al aplicar operaciones sobre la variable respuesta en la llamada a LM y si los ajustados son prediccinoes del transformado de la variable respuesta o predicciones de ella. Es una averiguación sencilla pero tengo sueño.  
...
* Probablemente sea mejor transformar explicadoras con un método análogo a PCA que seleccinoar modelo, para predicción. 
* Escalar las variables, son de escalas muy diferentes. 
